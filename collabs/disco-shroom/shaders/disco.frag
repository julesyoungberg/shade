...
void main() {
    ...
    pos = scale(pos, sin(u_time) * 0.2 + 0.8);
    
    float box = box(pos, vec2(0.7), 0.01);
    
    vec3 boxColor = vec3(1.0);
    boxColor.r *= sin(2.0 * u_time + tilePos.y * 4.0 + tilePos.x * 2.0) * 0.75 + 0.25;
    boxColor.g *= cos(1.5 * u_time + tilePos.y * 3.0 + tilePos.x * 4.0) * 0.75 + 0.25;

    vec3 bg = vec3(0.439, 0.031, 0.541);

    vec3 color = mix(bg, boxColor, box);
